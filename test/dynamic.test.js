const{expect}=require("chai");


const { network, deployments, ethers, getNamedAccounts } = require("hardhat");

const { developmentChains, networkConfig } = require("../helper-hardhat-config");

const lowSVGImageuri = 
"data:image/svg+xml;base64,"

const highSVGimageUri =
    "data:image/svg+xml;base64,"

    !developmentChains.includes(network.name)
    ? describe.skip
    : describe("Dynamic SVG NFT Unit Tests", function () {
          let dynamicSvgNft, deployer, mockV3Aggregator

          beforeEach(async () => {
              accounts = await ethers.getSigners()
              deployer = accounts[0];
              await deployments.fixture(["mocks", "dynamicsvg"])
              mockV3Aggregator = await ethers.getContract("MockV3Aggregator")
              dynamicSvgNft = await ethers.getContract("dynamicNFT")
            
          })

          describe("Constructor", ()=>{
            it("Sets everything properly",async()=>{
                const lowImg=await dynamicSvgNft.getLowUri();
                const highImg=await dynamicSvgNft.getHighUri();
    
                // console.log(lowImg, highImg);
    
                expect(highImg).to.be.equal(highSVGimageUri);
                expect(lowImg).to.be.equal(lowSVGImageuri);
            
            })

            it("Base Uri works properly", async()=>{
                const feed= await dynamicSvgNft.getpriceFeed();

                expect(feed.toString()).to.be.equal(mockV3Aggregator.address);

            })
        })
    })

    